...
import hardkernel.odroid.settings.MainSettings;
...
@Keep
public class PhysicalKeyboardFragment extends SettingsPreferenceFragment
    ...
    private void updateHardKeyboards(@NonNull List<HardKeyboardDeviceInfo> newHardKeyboards) {
        ...
        final PreferenceCategory category = new PreferenceCategory(getPreferenceManager().getContext());
        ...
        for (HardKeyboardDeviceInfo hardKeyboardDeviceInfo : newHardKeyboards) {
            ...
            pref.setLayoutResource(R.layout.leanback_preference);// default use framework/base/core/res/res/layout/preference.xml ,now use frameworks/support/leanback-preference/src/main/res/layout/leanback_preference.xml
    ...
    private void showKeyboardLayoutDialog(InputDeviceIdentifier inputDeviceIdentifier) {
        ...
        fragment.show(getFragmentManager(), KeyboardLayoutDialogFragment.class.getSimpleName());
    }
    ...
    private void updateShowVirtualKeyboardSwitch() {
        mShowVirtualKeyboardSwitch.setChecked(
                Secure.getInt(getActivity().getContentResolver(), Secure.SHOW_IME_WITH_HARD_KEYBOARD, 0) != 0);
    }
    ...
    private final OnPreferenceChangeListener mShowVirtualKeyboardSwitchPreferenceChangeListener =
            (preference, newValue) -> {
                Secure.putInt(getActivity().getContentResolver(), Secure.SHOW_IME_WITH_HARD_KEYBOARD,
