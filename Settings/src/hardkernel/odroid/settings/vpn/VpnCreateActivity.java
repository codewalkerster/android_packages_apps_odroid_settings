 ...
 *
 */
package hardkernel.odroid.settings.vpn;
...
import hardkernel.odroid.settings.BaseInputActivity;
...
import hardkernel.odroid.settings.utils.AndroidKeystoreAliasLoader;
...
public class VpnCreateActivity extends BaseInputActivity implements TextWatcher,
    ...
    final AndroidKeystoreAliasLoader androidKeystoreAliasLoader = new AndroidKeystoreAliasLoader(null);
    ...
    @Override
    public void init() {
        mService = getSystemService(VpnManager.class);
        ...
        loadCertificates(mIpsecUserCert, androidKeystoreAliasLoader.getKeyCertAliases(), 0, mProfile.ipsecUserCert);
    ...
    public void onClick(View view) {
        ...
        if (view == mBtnOK) {
            ...
            final boolean shouldLockdown = isVpnAlwaysOn();
            final boolean shouldConnect = shouldLockdown || isEditing();
            final boolean wasLockdown = VpnUtils.isAnyLockdownActive(this);
            try {
                final boolean replace = VpnUtils.isVpnActive(this);
                if (shouldConnect && !isConnected(profile) &&
                        ConfirmLockdownFragment.shouldShow(replace, wasLockdown, shouldLockdown)) {
                    ConfirmLockdownFragment mConfirmLockdownFragment = ConfirmLockdownFragment.newInstance();
                    final Bundle opts = new Bundle();
                    opts.putParcelable(ARG_PROFILE, profile);
                    ConfirmLockdownFragment.show(mConfirmLockdownFragment, replace, /* alwaysOn */ shouldLockdown,
    ...
    private void loadCertificates(Spinner spinner, Collection<String> choices, int firstId, String selected) {
        Context context = this;
    ...
    private boolean disconnect(VpnProfile profile) {
        try {
            ...
            return VpnUtils.disconnectLegacyVpn(this);
        } catch (RemoteException e) {
