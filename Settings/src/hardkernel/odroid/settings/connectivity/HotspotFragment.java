...
public class HotspotFragment extends SettingsPreferenceFragment
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mTm = (TetheringManager) getActivity().getSystemService(Context.TETHERING_SERVICE);
...
//        setPreferencesFromResource(R.xml.hotspot, null);
    ...
    private void initWifiTethering() {
        ...
        if (mWifiConfig == null) {
            ...
            mCreateNetwork.setSummary(String.format(activity.getString(CONFIG_SUBTEXT),
                    s, mSecurityType[HotspotDialogActivity.OPEN_INDEX]));
        } else {
            int index = HotspotDialogActivity.getSecurityTypeIndex(mWifiConfig);
            mCreateNetwork.setSummary(String.format(activity.getString(CONFIG_SUBTEXT),
                    mWifiConfig.getSsid(),
                    mSecurityType[index]));
        }
    ...
    private void updateState() {
        ...
        String[] errored = mTm.getTetheringErroredIfaces();
    ...
    private void updateUsbState(String[] available, String[] tethered,
        ...
        int usbError = TetheringManager.TETHER_ERROR_NO_ERROR;
        for (String s : available) {
            for (String regex : mUsbRegexs) {
                if (s.matches(regex)) {
                    if (usbError == TetheringManager.TETHER_ERROR_NO_ERROR) {
    ...
    //         mWifiConfig = HotspotDialogActivity.getConfig();
    //         if (mWifiConfig != null) {
    //             /**
    ...
    private static final class OnStartTetheringCallback extends
        ...
        final WeakReference<HotspotFragment> mTetherSettings;

        OnStartTetheringCallback(HotspotFragment settings) {
            mTetherSettings = new WeakReference<HotspotFragment>(settings);
        }
        ...
        private void update() {
            HotspotFragment settings = mTetherSettings.get();
            if (settings != null) {
